...
class ConnectionMixin:
            ...
            try:
                ...
                if not self._generation_is_current(connection_key, generation):
                    logger.info(
                        ...
                        f"generation={generation}, error={e}"
                    )
                    return
    ...
    async def _safe_close_websocket(self, ws: Any, timeout: float = 5.0):
        ...
        try:
            ...
            self.logger.warning("关闭 WebSocket 超时: close")
        except Exception as e:
            self.logger.debug(f"关闭 WebSocket 失败: {e}")
        ...
        try:
            ...
            self.logger.warning("关闭 WebSocket 超时: wait_closed")
        except Exception as e:
            self.logger.debug(f"等待 WebSocket 关闭失败: {e}")
