...
class LocalModelConfig:
    ...
    model_name: str = "customer-service:latest"
    max_tokens: int = 50
...
class AgentConfig:
    ...
    # LLM 配置 - 优先从 .env 读取
    model_name: str = field(default_factory=lambda: os.getenv("LLM_MODEL_NAME", get_config("llm.model_name", "customer-service:latest")))
    api_key: str = field(default_factory=lambda: os.getenv("LLM_API_KEY", get_config("llm.api_key", "ollama")))
    api_base: str = field(default_factory=lambda: os.getenv("LLM_API_BASE", get_config("llm.api_base", "http://localhost:11434/v1")))
    ...
    def load_from_config(cls) -> "AgentConfig":
        ...
        if local_model_cfg:
            config.local_model = LocalModelConfig(
                ...
                timeout=local_model_cfg.get("timeout", 60),
                local_max_length=get_config("routing.local_max_length", 15),
                fallback_on_error=get_config("routing.fallback_on_error", True),
            )

        # PyQt 设置页保存的 prompt 存在 ConfigManager/Redis 中，优先使用 UI 配置。
        try:
            from core.config_manager import config_manager

            ui_instructions = config_manager.get_prompt_instructions("default")
            if ui_instructions:
                config.instructions = ui_instructions
        except Exception as e:
            logger.warning(f"加载 UI Prompt 配置失败，继续使用 config.json: {e}")
