...
PRODUCT_CLARIFICATION_REPLY = "请问您要咨询哪款商品呢？可以发一下商品卡片或商品名称。"
...
class LiveChatService:
    """No-send InternalEngine live chat facade for Web Admin.

    The service keeps only in-process session state and never sends to PDD,
    calls external legacy backends, writes SQLite, writes pgvector, or exposes
    credentials.
    """
    ...
    def send_message(self, session_id: str, payload: LiveChatMessageRequest) -> LiveChatMessageResponse:
        ...
        adapter_trace_patch: dict[str, Any] = {
            ...
            "provider_status": _facade_provider_status(payload),
            "provider_error_type": "",
            "provider_error_summary": "",
            "stage_status": _facade_stage_status(),
            "latency_ms": _facade_latency(),
        }
        ...
        trace = {
            ...
            "intent_classifier_status": "skipped" if not payload.use_real_llm else "real_provider_not_connected_in_mvp",
        ...
        *,
        ...
        if _is_product_question(message):
            ...
            return (
                ...
                "mock_hit" if payload.use_rag else "disabled",
    ...
    def _product_reply(message: str, product_context: dict[str, Any]) -> str:
        ...
        if _contains_any(message, ("多少钱", "价格", "价钱")):
            if price:
                return f"亲，这款{name}页面价约{price}，实际以商品页和结算页为准哦。"
            return f"亲，这款{name}价格请以商品页和结算页显示为准哦。"
        if _contains_any(message, ("怎么用", "用法", "使用")):
            return f"亲，这款{name}按商品详情页说明使用就可以，具体用量以页面提示为准哦。"
    ...
    def _policy_chunks(domain: str) -> list[dict[str, Any]]:
        content_by_domain = {
            ...
            "after_sales_evidence": "售后问题先收集问题照片、外包装照片、订单信息和具体情况，不承诺结果。",
...
def _facade_provider_status(payload: LiveChatMessageRequest) -> dict[str, str]:
    return {
        "real_engine": "skipped",
        "pgvector": "disabled",
        "ollama": "disabled",
        "llm": "disabled",
        "intent_classifier": "disabled" if not payload.use_llm_intent_classifier else "facade",
        "answer_generator": "disabled" if not payload.use_llm_answer_generator else "facade",
    }
...
def _facade_stage_status() -> dict[str, str]:
    return {
        "context": "ok",
        "routing": "ok",
        "rag": "facade",
...
def _facade_latency() -> dict[str, int]:
