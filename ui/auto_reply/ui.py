# 自动回复主界面模块 - V2.0 Playwright 集成版
#
# 新增功能：
# - Playwright 桥接控制器集成
# - 优雅销毁机制（安全关闭 Playwright browser）
...
class AutoReplyUI(QFrame):
    ...
    def _on_playwright_status_changed(self, status_code: int, message: str):
        ...
        Args:
            status_code: 状态码 (0=断开, 1=连接中, 2=已连接, 3=错误)
            message: 状态消息
        """
        from services.pdd_protocol_service import PDDProtocolStatus

        status_name = PDDProtocolStatus(status_code).name
        self.logger.info(f"Playwright 状态变化: {status_name} - {message}")
